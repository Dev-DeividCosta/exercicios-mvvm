...
import com.example.myapplication.ui.screens.ClientScreen
import com.example.myapplication.ui.screens.SaleScreen
import com.example.myapplication.ui.screens.RouteScreen // <--- IMPORT IMPORTANTE
import com.example.myapplication.ui.screen.ScreenProdutos
import kotlinx.coroutines.launch
...
fun AppNavigator() {
    val items = remember {
        listOf(
            BottomNavItem.Charges, // Cobranças (Rota)
            BottomNavItem.Sales,   // Vendas
            BottomNavItem.Clients, // Clientes
            BottomNavItem.Products // Produtos
        )
    }

    val pagerState = rememberPagerState(
        initialPage = items.indexOf(BottomNavItem.Sales), // Começa na Venda
        pageCount = { items.size }
    )
    val scope = rememberCoroutineScope()
    ...
    Scaffold(
        ...
        HorizontalPager(
            ...
            modifier = Modifier.fillMaxSize().padding(paddingValues),
            key = { index -> items[index].route },
            userScrollEnabled = false // Trava o scroll lateral manual
        ) { pageIndex ->
            when (items[pageIndex]) {
                BottomNavItem.Clients -> ClientScreen()
                BottomNavItem.Products -> ScreenProdutos(onNavigateToProductDetail = {})
                BottomNavItem.Sales -> SaleScreen()

                // --- AQUI ESTAVA O PlaceholderScreen, AGORA É A TELA REAL ---
                BottomNavItem.Charges -> RouteScreen()
            }
        }
    }
}
